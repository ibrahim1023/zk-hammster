        ...
        meta.create_gate("xor", |meta| {
            ...
            vec![
                s_xor *
                    (lhs.clone() +
                        rhs.clone() -
                        Expression::Constant(F::ONE.double()) * lhs * rhs -
                        out)
            ]
        });
...
pub fn run_mock_prover(k: u32, circuit: &HammsterCircuit<Fp>, pub_input: &Vec<Fp>) {
    let prover = MockProver::run(k, circuit, vec![pub_input.clone()]).expect(
        "Mock prover should run"
    );
...
pub fn generate_proof(
    ...
    create_proof(params, pk, &[circuit], &[&[pub_input]], OsRng, &mut transcript).expect(
        "Prover should not fail"
    );
    transcript.finalize()
}
